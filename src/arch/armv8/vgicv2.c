...
static void vgicd_emul_sgiregs_access(struct vcpu* vcpu, struct emul_access* acc, struct vgic_reg_handler_info* handlers,
    bool gicr_access, vcpuid_t vgicr_id)
{
    ...
    uint32_t val = acc->write ? (uint32_t)vcpu_readreg(vcpu, acc->reg) : 0;
...
void vgic_init(struct vm* vm, const struct vgic_dscrp* vgic_dscrp)
{
    ...
    vm->arch.vgic.vgicd.interrupts =
        mem_alloc_page(NUM_PAGES(vgic_int_size), SEC_HYP_GLOBAL, MEM_ALIGN_NOT_REQ);
    ...
    for (irqid_t i = 0; i < vm->arch.vgic.vgicd.int_num; i++) {
        ...
        vm->arch.vgic.vgicd.interrupts[i].targets = 0;
