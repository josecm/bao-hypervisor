...
uint8_t gicr_get_prio(irqid_t int_id, cpuid_t gicr_id)
{
    ...
    uint8_t prio = (uint8_t) ((gicr[gicr_id].IPRIORITYR[reg_ind] >> off) &
        BIT32_MASK(off, GIC_PRIO_BITS));
...
void gicr_set_icfgr(irqid_t int_id, uint8_t cfg, cpuid_t gicr_id)
{
    ...
    if (reg_ind == 0) {
        gicr[gicr_id].ICFGR0 =
            (gicr[gicr_id].ICFGR0 & ~mask) | ((((uint32_t)cfg) << off) & mask);
    } else {
        gicr[gicr_id].ICFGR1 =
            (gicr[gicr_id].ICFGR1 & ~mask) | ((((uint32_t)cfg) << off) & mask);
    }
