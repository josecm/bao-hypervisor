...
uint8_t vgic_int_ptarget_mask(struct vcpu* vcpu, struct vgic_int* interrupt)
{
    if (vgic_broadcast(vcpu, interrupt)) {
        return (uint8_t)(vcpu->vm->cpus & ~(1U << vcpu->phys_id));
    } else {
...
static void vgicr_emul_ctrl_access(struct vcpu* vcpu, struct emul_access* acc, struct vgic_reg_handler_info* handlers,
...
static void vgicr_emul_typer_access(struct vcpu *vcpu, struct emul_access* acc, struct vgic_reg_handler_info* handlers,
    bool gicr_access, vcpuid_t vgicr_id)
{
    UNUSED_ARG(handlers);
    UNUSED_ARG(gicr_access);
    ...
    if (!acc->write) {
        ...
        if (trgt_vcpu != NULL) {
            ...
            vcpu_writereg(vcpu, acc->reg, (unsigned long)typer);
        }
    }
}

static void vgicr_emul_pidr_access(struct vcpu* vcpu, struct emul_access* acc, struct vgic_reg_handler_info* handlers,
    bool gicr_access, vcpuid_t vgicr_id)
{
    UNUSED_ARG(handlers);
    UNUSED_ARG(gicr_access);
...
static void vgicd_emul_router_access(struct vcpu *vcpu, struct emul_access* acc, struct vgic_reg_handler_info* handlers,
    bool gicr_access, vcpuid_t vgicr_id)
{
    bool word_access = (acc->width == 4);
    bool top_access = word_access && ((acc->addr & 0x4) != 0);
    ...
    struct vgic_int* interrupt = vgic_get_int(vcpu, (irqid_t)irq_id, vcpu->id);
    ...
    if (!acc->write) {
        ...
        vgic_int_set_field(handlers, vcpu, interrupt, (unsigned long)route);
    }
}
...
static bool vgicr_emul_handler(struct vcpu *vcpu, struct emul_access* acc)
{
    ...
    if (vgic_check_reg_alignment(acc, handler_info)) {
        ...
        handler_info->reg_access(vcpu, acc, handler_info, VGIC_GICR_ACCESS, vgicr_id);
...
static bool vgic_icc_sgir_handler(struct vcpu* vcpu, struct emul_access* acc)
{
    if (acc->write) {
        ...
        if (sgir & ICC_SGIR_IRM_BIT) {
             ...
             * cluster support.
             */
            trgtlist = vm_translate_to_pcpu_mask(vcpu->vm,
                (cpumap_t)ICC_SGIR_TRGLSTFLT(sgir), vcpu->vm->cpu_num);
        }
...
static bool vgic_icc_sre_handler(struct vcpu* vcpu, struct emul_access* acc)
{
    if (!acc->write) {
...
void vgic_init(struct vm* vm, const struct vgic_dscrp* vgic_dscrp)
{
    ...
    vm->arch.vgic.vgicd.interrupts =
        mem_alloc_page(NUM_PAGES(vgic_int_size), SEC_HYP_GLOBAL, false);
    if (vm->arch.vgic.vgicd.interrupts == NULL) {
        ERROR("failed to alloc vgic");
    }

    for (irqid_t i = 0; i < vm->arch.vgic.vgicd.int_num; i++) {
...
void vgic_cpu_init(struct vcpu* vcpu)
{
    ...
    for (irqid_t i = 0; i < GICH_MAX_NUM_LRS; i++) {
