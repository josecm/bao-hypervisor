...
static inline 
uint8_t gich_lr_state(gic_lr_t lr)
{
    return (uint8_t)GICH_LR_STATE(lr);
}

static inline bool gich_lr_valid(gic_lr_t lr)
{
    return gich_lr_state(lr) != 0;
}
...
static inline void vgic_write_gich_lr(struct vcpu *vcpu, size_t lr_id, gic_lr_t lr_val)
{
    if (vgic_vcpu_is_current(vcpu)) {
        ...
        if (gich_lr_valid(lr_val)) {
            vcpu->arch.vgic_priv.gich.elrsr = bit64_set(vcpu->arch.vgic_priv.gich.elrsr, lr_id);
        } else {
            vcpu->arch.vgic_priv.gich.elrsr = bit64_clear(vcpu->arch.vgic_priv.gich.elrsr, lr_id);
        }
        // TODO: manage ap1r state as well
    }
}
...
static inline ssize_t gich_get_lr(struct vcpu* vcpu, struct vgic_int* interrupt, gic_lr_t* lr)
{
...
static inline uint8_t vgic_get_state(struct vcpu* vcpu, struct vgic_int* interrupt)
{
    uint8_t state = 0;

    gic_lr_t lr_val = 0;
    if (gich_get_lr(vcpu, interrupt, &lr_val) >= 0) {
        state = (uint8_t)GICH_LR_STATE(lr_val);
    } else {
...
void vgic_yield_ownership(struct vcpu* vcpu, struct vgic_int* interrupt)
{
    if ((GIC_VERSION == GICV2 && gic_is_priv(interrupt->id)) || !vgic_owns(vcpu, interrupt) ||
        interrupt->in_lr || (vgic_get_state(vcpu, interrupt) & ACT)) {
        return;
    }
...
static inline void vgic_write_lr(struct vcpu* vcpu, struct vgic_int* interrupt, size_t lr_ind)
{
    ...
    unsigned state = vgic_get_state(vcpu, interrupt);
    ...
    if (vgic_int_is_hw(interrupt)) {
        ...
        if (state & ACT) {
            lr |= (((gic_lr_t)interrupt->sgi.act) << GICH_LR_CPUID_OFF) & GICH_LR_CPUID_MSK;
            lr |= GICH_LR_STATE_ACT;
        } else {
    ...
    vgic_write_gich_lr(vcpu, lr_ind, lr);
}
    ...
    if (GICH_LR_STATE(lr_val) != INV) {
        ...
        if (interrupt->id < GIC_MAX_SGIS) {
            if (interrupt->state & ACT) {
                interrupt->sgi.act = (uint8_t)GICH_LR_CPUID(lr_val);
            } else if (interrupt->state & PEND) {
                interrupt->sgi.pend |= (uint8_t)(1U << GICH_LR_CPUID(lr_val));
            }
        }
#endif
        uint32_t hcr = vgic_get_gich_hcr(vcpu);
        if ((interrupt->state & PEND) && interrupt->enabled) {
            hcr |= GICH_HCR_NPIE_BIT;
        }
        vgic_set_gich_hcr(vcpu, hcr | GICH_HCR_UIE_BIT);
...
static void vgic_add_spilled(struct vcpu* vcpu, struct vgic_int* interrupt)
{
    ...
    vgic_set_gich_hcr(vcpu, vgic_get_gich_hcr(vcpu) | GICH_HCR_NPIE_BIT);
}

static void vgic_spill_lr(struct vcpu* vcpu, size_t lr_ind)
{
    gic_lr_t lr = vgic_read_gich_lr(vcpu, lr_ind);
...
static inline ssize_t gich_next_free_lr(struct vcpu* vcpu)
{
    uint64_t elrsr = vgic_get_gich_elrsr(vcpu);
    ssize_t lr_num = bit64_ffs(elrsr);
    ssize_t lr_id = -1;

    if (lr_num >= 0) {
        lr_id = (ssize_t)lr_num;
    }
...
bool vgic_add_lr(struct vcpu* vcpu, struct vgic_int* interrupt)
{
    ...
    ssize_t lr_ind = gich_next_free_lr(vcpu);
...
static inline void vgic_update_enable(struct vcpu* vcpu)
{
    ...
    if (vcpu->vm->arch.vgic.vgicd.CTLR & VGIC_ENABLE_MASK) {
        vgic_set_gich_hcr(vcpu, vgic_get_gich_hcr(vcpu) | GICH_HCR_En_BIT);
    } else {
        vgic_set_gich_hcr(vcpu, vgic_get_gich_hcr(vcpu) & ~GICH_HCR_En_BIT);
    }
}

static void vgicd_emul_misc_access(struct vcpu* vcpu, struct emul_access* acc,
    struct vgic_reg_handler_info* handlers, bool gicr_access, cpuid_t vgicr_id)
{
    UNUSED_ARG(handlers);
    UNUSED_ARG(gicr_access);
    UNUSED_ARG(vgicr_id);

    struct vgicd* vgicd = &vcpu->vm->arch.vgic.vgicd;
    ...
    switch (reg) {
        case GICD_REG_IND(CTLR):
            if (acc->write) {
                ...
                vgicd->CTLR = vcpu_readreg(vcpu, acc->reg) & VGIC_ENABLE_MASK;
                if (prev_ctrl ^ vgicd->CTLR) {
                    vgic_update_enable(vcpu);
                    struct cpu_msg msg = {
                        (uint32_t)VGIC_IPI_ID,
                        VGIC_UPDATE_ENABLE,
                        VGIC_MSG_DATA(vcpu->vm->id, 0, 0, 0, 0),
                    };
                    vm_msg_broadcast(vcpu->vm, &msg);
                }
            } else {
                vcpu_writereg(vcpu, acc->reg, vgicd->CTLR | GICD_CTLR_ARE_NS_BIT);
            }
            break;
        case GICD_REG_IND(TYPER):
            if (!acc->write) {
                vcpu_writereg(vcpu, acc->reg, vgicd->TYPER);
            }
            break;
        case GICD_REG_IND(IIDR):
            if (!acc->write) {
                vcpu_writereg(vcpu, acc->reg, vgicd->IIDR);
            }
            break;
        default:
            vgic_emul_razwi(vcpu, acc, handlers, gicr_access, vgicr_id);
            break;
    }
}

static void vgicd_emul_pidr_access(struct vcpu* vcpu, struct emul_access* acc, struct vgic_reg_handler_info* handlers,
    bool gicr_access, cpuid_t vgicr_id)
{
    ...
    if (!acc->write) {
        vcpu_writereg(vcpu, acc->reg, gicd->ID[((acc->addr & 0xff) - 0xd0) / 4]);
    }
}
...
void vgic_emul_generic_access(struct vcpu* vcpu, struct emul_access* acc, struct vgic_reg_handler_info* handlers,
    bool gicr_access, cpuid_t vgicr_id)
{
    ...
    if (valid_access) {
        for (size_t i = 0; i < ((acc->width * 8) / field_width); i++) {
            struct vgic_int* interrupt =
                vgic_get_int(vcpu, (irqid_t)(first_int + i), vgicr_id);
            if (interrupt == NULL) {
                break;
            }
            if (acc->write) {
                unsigned long data = bit_extract(val, i * field_width, field_width);
                vgic_int_set_field(handlers, vcpu, interrupt, data);
            } else {
                val |= (handlers->read_field(vcpu, interrupt) & mask) << (i * field_width);
            }
        }
    }

    if (!acc->write) {
        vcpu_writereg(vcpu, acc->reg, (unsigned long)val);
    }
}
...
bool vgicd_emul_handler(struct vcpu* vcpu, struct emul_access* acc)
{
    ...
    if (vgic_check_reg_alignment(acc, handler_info)) {
        spin_lock(&vcpu->vm->arch.vgic.vgicd.lock);
        handler_info->reg_access(vcpu, acc, handler_info, VGIC_NOT_GICR_ACCESS, vcpu->id);
        spin_unlock(&vcpu->vm->arch.vgic.vgicd.lock);
        return true;
    } else {
        return false;
    }
...
void vgic_ipi_handler(uint32_t event, uint64_t data)
{
    ...
    if((cpu()->vcpu != NULL) && (vm_id == cpu()->vcpu->vm->id)){
        vcpu = cpu()->vcpu;
    } else {
        vcpu = cpu_get_vcpu_by_vmid(vm_id);
    }
    ...
    switch (event) {
        ...
        case VGIC_INJECT: {
            vgic_inject(vcpu, int_id, (vcpuid_t)val);
        } break;
        ...
        case VGIC_SET_REG: {
            ...
            if (handlers != NULL && interrupt != NULL) {
                vgic_int_set_field(handlers, vcpu, interrupt, (unsigned long)val);
            }
        } break;
    ...
    struct list** outlist)
{
    ...
    for (size_t i = 0; i < spilled_list_size; i++) {
        ...
        list_foreach ((*list), struct vgic_int, temp_irq) {
            if (!(vgic_get_state(vcpu, temp_irq) & flags)) {
                continue;
            }
...
static void vgic_refill_lrs(struct vcpu* vcpu, bool npie)
{
    ...
    while (lr_ind >= 0) {
        ...
        if (irq != NULL) {
            ...
            uint32_t hcr = vgic_get_gich_hcr(vcpu);
            vgic_set_gich_hcr(vcpu, hcr & ~(GICH_HCR_NPIE_BIT | GICH_HCR_UIE_BIT));
            break;
        }
        flags = ACT | PEND;
        lr_ind = gich_next_free_lr(vcpu);
    }
    spin_unlock(&vcpu->vm->arch.vgic.spilled_lock);
}
...
static void vgic_handle_trapped_eoir(struct vcpu* vcpu)
{
    ...
    while (lr_ind >= 0) {
        ...
        struct vgic_int* interrupt = vgic_get_int(vcpu, (irqid_t)(GICH_LR_VID(lr_val)), vcpu->id);
        if (interrupt == NULL) {
            continue;
        }
...
void vgic_hw_irq_save_state(struct vcpu* vcpu, irqid_t irq_id)
{
    ...
    if (interrupt->hw) {
        interrupt->state = vgic_get_state(vcpu, interrupt);
    }
}
