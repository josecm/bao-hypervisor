...
static gic_lr_t vgic_read_gich_lr(struct vcpu *vcpu, ssize_t lr_id)
{
    gic_lr_t lr_value;
...
static inline void vgic_write_gich_lr(struct vcpu *vcpu, ssize_t lr_id, gic_lr_t lr_val)
{
    if (vgic_vcpu_is_current(vcpu)) {
...
static inline int64_t gich_get_lr(struct vcpu* vcpu, struct vgic_int* interrupt, unsigned long* lr)
{
    if (!interrupt->in_lr || interrupt->owner->phys_id != cpu()->id) {
        return -1;
    }

    unsigned long lr_val = vgic_read_gich_lr(vcpu, interrupt->lr);
...
bool vgic_remove_lr(struct vcpu* vcpu, struct vgic_int* interrupt)
{
    ...
    if ((lr_ind = gich_get_lr(vcpu, interrupt, &lr_val)) >= 0) {
        vgic_write_gich_lr(vcpu, lr_ind, 0);
    }
...
{
    unsigned long lr = vgic_read_gich_lr(vcpu, lr_ind);
...
bool vgic_add_lr(struct vcpu* vcpu, struct vgic_int* interrupt)
{
    ...
    if (lr_ind < 0) {
        ...
        for (size_t i = 0; i < GIC_NUM_LRS; i++) {
            unsigned long lr = vgic_read_gich_lr(vcpu, i);
...
void vgicd_emul_misc_access(struct vcpu* vcpu, struct emul_access* acc,
...
void vgicd_emul_pidr_access(struct vcpu* vcpu, struct emul_access* acc, struct vgic_reg_handler_info* handlers,
    ...
    bool gicr_access, cpuid_t vgicr_id)
{
    ...
    if (valid_access) {
        for (size_t i = 0; i < ((acc->width * 8) / field_width); i++) {
            struct vgic_int* interrupt = vgic_get_int(vcpu, first_int + i, vgicr_id);
            if (interrupt == NULL) {
                break;
            }
            if (acc->write) {
...
bool vgicd_emul_handler(struct vcpu* vcpu, struct emul_access* acc)
{
    ...
    if (vgic_check_reg_alignment(acc, handler_info)) {
        ...
        handler_info->reg_access(vcpu, acc, handler_info, false, vcpu->id);
...
void vgic_handle_trapped_eoir(struct vcpu* vcpu)
{
    ...
    while (lr_ind >= 0) {
        unsigned long lr_val = vgic_read_gich_lr(vcpu, lr_ind);
        vgic_write_gich_lr(vcpu, lr_ind, 0);
