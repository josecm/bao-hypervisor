...
static void vm_master_init(struct vm* vm, const struct vm_config* vm_config, vmid_t vm_id)
{
    vm->master = cpu()->id;
    vm->config = vm_config;
    vm->cpu_num = convm_configfig->platform.cpu_num;
    vm->id = vm_id;
...
static void vm_init_mem_regions(struct vm* vm, const struct vm_config* vm_config)
{
    for (size_t i = 0; i < vm_config->platform.region_num; i++) {
        ...
        bool img_is_in_rgn = range_in_range(
            vm_config->image.base_addr, vm_config->image.size, reg->base, reg->size);
        if (img_is_in_rgn) {
...
struct vm* vm_init(struct vm_allocation* vm_alloc, const struct vm_config* vm_config,
